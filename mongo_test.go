...
func TestQuerySeek(t *testing.T) {
	...
	for _, d := range data {
		err := ds.Put(context.TODO(), datastore.NewKey(d.key), d.value)
	...
	for i, q := range cases {
		t.Run(fmt.Sprintf("%d", i+1), func(t *testing.T) {
			...
			res, err := ds.QueryExtended(context.TODO(), q)
			require.NoError(t, err)
			all, err := res.Rest()
			require.NoError(t, err)
...
func TestTxnDiscard(t *testing.T) {
	...
	txn, err := ds.NewTransaction(context.TODO(), false)
	if err != nil {
		t.Fatal(err)
	}
	key := datastore.NewKey("/test/thingdiscard")
	if err := txn.Put(context.TODO(), key, []byte{1, 2, 3}); err != nil {
		t.Fatal(err)
	}
	txn.Discard(context.TODO())
	has, err := ds.Has(context.TODO(), key)
	if err != nil {
		t.Fatal(err)
	}
	if has {
...
func TestTxnBatch(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		err = txn.Put(context.TODO(), key, bytes)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	for key, bytes := range data {
		retrieved, err := ds.Get(context.TODO(), key)
		if err != nil {
			t.Fatal(err)
		}
...
func createMongoDS(t *testing.T, uri string) *MongoDS {
	ds, err := New(context.TODO(), uri, randStoreName())
	require.NoError(t, err)
	return ds
}
